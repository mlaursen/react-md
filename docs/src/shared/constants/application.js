...
const buildURI = (protocol, host, port, path) => `${protocol}://${host}${__DEV__ && port ? `:${port}` : ''}${path}`;
...
export const API_URL = buildURI(apiProtocol, apiHost, apiPort, apiPath);

export const BASE_URL = buildURI(protocol, host, port, path);
