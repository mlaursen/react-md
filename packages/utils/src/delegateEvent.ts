...
export interface IDelegatableEvent {
  type: string;
...
const delegatedEvents: IDelegatableEvent[] = [];

/**
 * Creates the delegated event handler that will run all the callbacks once
 * an event happens. The callbacks' invocation can also be throttled for
 * event types that trigger rapidly for additional performance.
 *
 ...
 * functions once you have created this event handler. Otherwise there'd be
 * ts errors trying to do `MouseEvent` or `KeyboardEvent` in your listeners.
 */
...
/**
 * Creates a delegated event listener using custom events. Most of this code comes
 ...
 * that is added will be called with the event each time the event is triggered. This does
 * mean that you will manually need to remove your callback like normal or else it can be
 * called when no longer in use. This also means that it doesn't "hurt" to call this
 * function without immediately calling the `add` function since the event won't start until
 * there is at least 1 callback.
 *
 * @see https://developer.mozilla.org/en-US/docs/Web/Events/resize#Examples
 * @param eventType - One of the event types that should be used to create a delegated
 * event for. This should be things like resize, click, scroll, etc.
 ...
 * @param throttle - Boolean if the event should be throttled or not. Normally only event
 * types like resize or scroll should be throttled for performance boosts, but anything
 * can be.
