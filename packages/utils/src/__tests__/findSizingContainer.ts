import findSizingContainer, { SizingContainerSelectorError } from "../findSizingContainer";
...
describe("findSizingContainer", () => {
  ...
  describe("react-md component fixes", () => {
    it(
      "should attempt to get the 'rmd-tree-item__content' child element if the role is a treeitem because the entire " +
        "treeitem size would include nested groups which will cause invalid calclations for positioning",
      () => {
    ...
    it(
      "should attempt to get the 'rmd-item-text' child element when the role is a listitem because the entire " +
        "listitem size might include children that should not be included in the sizing",
      () => {
  ...
  it("should log an error in the console if the 'data-sizing-selector' did not return an element", () => {
    const el = document.createElement("div");
    const sizer1 = document.createElement("div");
    sizer1.className = "classname-query";
    ...
    el.setAttribute("data-sizing-selector", ".no-match-query");
    expect(() => findSizingContainer(el)).toThrow(
      new SizingContainerSelectorError(".no-match-query", el)
    );
  });
});
