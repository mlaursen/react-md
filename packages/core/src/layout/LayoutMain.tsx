...
export interface LayoutMainProps extends HTMLAttributes<HTMLDivElement> {
   ...
   *
   * @defaultValue `DEFAULT_LAYOUT_MAIN_CLASSNAMES`
   * @see {@link DEFAULT_LAYOUT_MAIN_CLASSNAMES}
   */
...
export const LayoutMain = forwardRef<HTMLDivElement, LayoutMainProps>(
  function LayoutMain(props, nodeRef) {
    const {
      className,
    ...
    if (mode === "keyboard" && typeof propTabIndex === "undefined") {
      tabIndex = -1;
    }
    ...
    return <Component {...remaining} {...elementProps} tabIndex={tabIndex} />;
  }
);
