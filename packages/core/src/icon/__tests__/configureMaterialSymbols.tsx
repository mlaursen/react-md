...
const configureIconsMock = vi.mocked(configureIcons);

describe("configureMaterialSymbols", () => {
  it("should mutate the `ICON_CONFIG` with material symbol defaults", async () => {
    expect(configureIconsMock).not.toHaveBeenCalled();

    await import("../configureMaterialSymbols.js");

    expect(configureIconsMock).toHaveBeenCalledWith({
