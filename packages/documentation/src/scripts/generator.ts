...
commander.command("template [packages...]")
  ...
  .action((packages, command) => {
    const { sassdoc: includeSassDoc, proptypes: includePropTypes, examples } = command as ITemplateOptions;
...
commander.command("sassdoc [options]")
  .option("--no-clean", "Updates the command so that the temp folder of styles will not be removed after being run")
  .action((_, command) => {
