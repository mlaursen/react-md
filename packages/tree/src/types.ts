...
export interface TreeItemIds {
  ...
  /**
   * This id is used to be able to link tree items together with children and parents. If the `parentId`
   * is set to `null`, the tree item will appear at the root of the tree. Otherwise, the tree item
   * will appear as a child of the referenced tree item by `itemId`
   */
...
export interface TreeItemExpansionIcon {
   ...
   *
   * NOTE: This will override the `rightIcon`/`rightAvatar` props if defined on each item by default
   * so they can't be used. If the `expanderLeft` prop is enabled, the `leftIcon`/`leftAvatar` will
...
export interface TreeItemExpansion {
  /**
   * The list of expanded ids within the tree. These ids should reference `itemId`s for tree items.
   */
  ...
  /**
   * A function to call when the user presses the asterisk key (*) that will expand all tree items
   * at the same level as the currently focused item.
   */
...
export interface TreeItemSelection {
  ...
  /**
   * The list of selected ids within the tree. If you only want to allow a single item to be selected
   * at a time within a tree, keep this as either an empty list (no selections) or a single `itemId`.
   */
  selectedIds: SelectedIds;

  /**
   * A function to call that will update the `selectedIds` to include `itemId` of the newly selected
   * tree item. This will be triggered when:
   * - the user clicks a tree item with mouse or touch
   * - user presses the space or enter key while keyboard focusing a tree item
   *
   * Note: If you are using the `useTreeItemSelection` hook, this will always cause the `selectedIds`
   * to be a list of just the selected `itemId` unless the `multiSelect` (second argument) is enabled.
   */
  onItemSelect: (itemId: TreeItemId) => void;

  /**
   * A function to  call that will update the `selectedIds` for "batch" selection updates. This will
   * always be called with a unique list of `itemId`s that contained the previous `itemId`s including
   * the new `itemId`s. This will only be called when the `multiSelect` prop has been enabled and:
...
export interface TreeItemProps
  ...
  /**
   * A DOM id for the tree item. This is required for a11y since this id should reflect the
   * `aria-activedescendant` value on the base `Tree` component to show current keyboard focus.
   */
  id: string;
  ...
  /**
   * An optional `style` to apply to the surrounding `<li>` element for the tree item. The `style` prop
   * is actually passed to the `contentComponent` element instead since you normally don't want to apply
   * styles to the surrounding element.
   */
  liStyle?: CSSProperties;

  /**
   * An optional `className` to apply to the surrounding `<li>` element for the tree item. The `className`
   * prop is actually passed to the `contentComponent` element instead since you normally don't want to apply
   * styles to the surrounding element.
   */
  liClassName?: string;
   ...
   * some false-positives, you can set this flag instead which will always be used instead.
   */
  isLink?: boolean;

  /**
   * The component to render the item's content in. This should normally stay as the default `"span"` element,
   * but if you'd like to make a navigation tree, this can be set as a `Link` component from a routing library.
   *
   * Example:
   * ```
...
export interface ProvidedTreeItemProps
  ...
  /**
   * React `key`s aren't really "props", but it will be provided to each `TreeItem` automatically.
   */
  key: string;
