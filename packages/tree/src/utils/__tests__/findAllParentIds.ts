...
describe("findAllParentIds", () => {
  ...
  it("should find all parent ids for a single id", () => {
    ...
    expect(findAllParentIds(SIMPLE_NESTED_LIST, ["item-1-1"])).toEqual(
      expected1
    );
    expect(findAllParentIds(SIMPLE_NESTED_LIST, ["item-1-1-2"])).toEqual(
      expected2
    );
    expect(findAllParentIds(SIMPLE_NESTED_LIST, ["item-1-3-1"])).toEqual(
      expected3
    );
  });
  ...
  it("should fild all parent ids for multiple unique ids", () => {
    ...
    expect(
      findAllParentIds(SIMPLE_NESTED_LIST, ["item-1-1", "item-3-1"])
    ).toEqual(expected1);
  });
  ...
  it("should fild all parent ids for multiple ids without including duplicates", () => {
    ...
    expect(
      findAllParentIds(SIMPLE_NESTED_LIST, ["item-1-1", "item-1"])
    ).toEqual(expected1);
    expect(
      findAllParentIds(SIMPLE_NESTED_LIST, ["item-1-1-2", "item-1"])
    ).toEqual(expected2);
  });
});
