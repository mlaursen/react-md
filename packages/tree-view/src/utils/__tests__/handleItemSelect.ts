...
describe("handleItemSelect", () => {
  ...
  describe("multi-selection", () => {
    it("should add the itemId to the list if it did not already exist", () => {
      ...
      expect(handleItemSelect(ITEM_ID_2, [ITEM_ID_1], true)).toEqual([ITEM_ID_1, ITEM_ID_2]);
    });
    ...
    it("should should remove the itemId from the list if it already existed", () => {
      ...
      expect(handleItemSelect(ITEM_ID_1, [ITEM_ID_1, ITEM_ID_2], true)).toEqual([ITEM_ID_2]);
      expect(handleItemSelect(ITEM_ID_1, [ITEM_ID_2, ITEM_ID_1], true)).toEqual([ITEM_ID_2]);
    });
  });
});
