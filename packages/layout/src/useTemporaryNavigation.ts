...
/**
 * This hook allows the temporary navigation pane to automatically close whenever
 * the route changes (so basically a user clicked one of the nav items). This
 * is normally the desired behavior for a temporary navigation pane, but it can be
 * disabled and a custom implementation can be used with the `useNavigationVisibility`
 * hook.
 *
 * Note: This hook assumes that the `selectedIds` array will only ever have a single
 * `selectedId` and the `selectedId` changes when the route changes since that is how
 * the `useLayoutNavigation` hook works.
 *
