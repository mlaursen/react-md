...
function isInterfaceMatched({ names, declaration }: IsMatchedOptions): boolean {
  return (
...
function isDeclarationMatched(
  ...
  return !!(
    ...
    (isTypeAliasMatched({ declaration, names: types }) ||
      isInterfaceMatched({ declaration, names: types }) ||
...
export async function parseAliasedFiles(
  ...
  // ERROR IN TABLE AND EXPANSION_PANEL
  if (movedImports.length !== vars.size + types.size) {
    /* eslint-disable no-console */
    const r = [...vars, ...types].map((name) => `- ${name}`).join("\n");
    console.log(`Error in "${importPath}"`);
    console.log("Expected");
    console.log(r);
    console.log();
    console.log("Got");
    console.log(movedImports);
    console.log();

    throw new Error(`Missing demo imports in ${importPath}`);
  }
}
