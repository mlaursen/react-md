...
export interface ITooltipDefaultProps
  extends Omit<ITooltipBaseDefaultProps, "position"> {
  position: "auto" | TooltipPosition;
  delay: number;
  vwMargin: number;
  vhMargin: number;
...
> {
  ...
  public static getDerivedStateFromProps(
    nextProps: ITooltipProps,
    prevState: ITooltipState
  ) {
    if (
      nextProps.position !== "auto" &&
      prevState.position !== nextProps.position
    ) {
  ...
  private getSpacing = () => {
    const { spacing, denseSpacing, dense } = this
      .props as TooltipWithDefaultProps;
    ...
    this.timeout = window.setTimeout(
      () => {
        if (this.container && process.env.NODE_ENV !== "production") {
          ...
          if (
            tooltip &&
            this.container.contains(tooltip) &&
            position !== "relative"
          ) {
            /* tslint:disable no-console */
            console.error(
  ...
  private updatePosition = (node: HTMLSpanElement | null) => {
    ...
    const style = positionRelativeTo(
      this.container,
      node,
      this.createPositionOptions(position)
    );
