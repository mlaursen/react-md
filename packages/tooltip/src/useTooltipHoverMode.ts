...
interface TooltipHoverModeState {
  ...
  /**
   * This will enable the immediate tooltip visibility functionality by updating the
   * `delay` state to be `0`. This should normally be called once a tooltip becomes
   * visible.
   */
  enable: () => void;

  /**
   * Whenever a tooltip loses its visibility, this function could be called to start
   * a timer that will disable the immediate tooltip visibility once complete. If
   * another tooltip becomes visible, this timeout is automatically cleared by the
   * `enable` function.
   */
...
/**
 * This keeps track of the current "immediate" delay to use for all child tooltip components.
 * When the `enable` function is called, this is updated to be `0` so tooltips can appear
...
/**
 * Keeps track of if the immediate mode is enabled. This allows for easy configuration of
 * the app by automatically enabling hover mode functionality if the `ImmediateTooltipConfig`
 * is in your app.
 */
...
/**
 * This hook returns the actions to enable the hover mode as well as start a timer
 * to disable it.
 * @private
 */
