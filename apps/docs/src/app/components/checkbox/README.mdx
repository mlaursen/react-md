...
<CodeLoader source="./CheckboxSizes.tsx" importUrl={import.meta.url} editable />

## Checkbox States

The `Checkbox` also has different states that can be applied. The most common
will be the `error` or `disabled` states but also supports `readOnly` and
`active`.

<CodeLoader
  source="./CheckboxStates.tsx"
  importUrl={import.meta.url}
  editable
/>
...
<CodeLoader
  source="./ControllingCheckboxes.tsx"
  importUrl={import.meta.url}
  editable
/>
...
state of a checkbox group. The hook requires a `name` to be provided and an
optional list of `defaultCheckedValues` returning an object containing:
...
<CodeLoader
  source="./UsingTheUseCheckboxGroupHook.tsx"
  importUrl={import.meta.url}
  editable
/>

### Strictly Typed useCheckboxGroup

For `Typescript` users, the `checkboxValues` will be strictly typed either by
inferring the type from the `defaultCheckedValues` or by providing a
type-parameter to the hook.

<CodeLoader
  source="./StrictlyTypedUseCheckboxGroup.tsx"
  importUrl={import.meta.url}
/>
...
<CodeLoader
  source="./IndeterminateCheckbox.tsx"
  importUrl={import.meta.url}
  editable
/>
...
A group of checkboxes can be controlled by using the `useCheckboxGroup` hook
with or without the indeterminate checkbox. The hook requires a `name` which
will be applied to each checkbox and a list of available `values`.

> !Info! The values will be strictly typed and inferred for `Typescript` users
> based on the provided list of available `values`.

<CodeLoader
  source="./CheckboxGroupHook.tsx"
  importUrl={import.meta.url}
  editable
/>
