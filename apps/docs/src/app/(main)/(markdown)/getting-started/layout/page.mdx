...
## Updating the layout to be temporary until a specific breakpoint (Optional)

When using the [useExpandableLayout](/hooks/use-expandable-layout) or
[useResizableLayout](/hooks/use-resizable-layout), the default
behavior is to use the temporary navigation until the [tablet
breakpoint](/customization/breakpoints#configuration). This can be configured by
updating the `temporaryUntil` option and the
[$SASSDOC]($layout-navigation-breakpoint) Sass variable.

### Temporary Until Desktop

Start by updating the Sass variables:
...
Then update the `useExpandableLayout` or `useResizableLayout` hook to include
the `temporaryUntil` breakpoint:
...
```diff
 export function ExpandableLayoutExample(
   ...
   const {
     ...
     expandableNavProps,
-  } = useExpandableLayout({ pathname });
+  } = useExpandableLayout({ pathname, temporaryUntil: "desktop" });
```

Here's the same thing, but for the `useResizableLayout`:
...
```diff
 export function ResizableLayoutExample(
   ...
   const {
     ...
     windowSplitterProps,
-  } = useResizableLayout({ pathname });
+  } = useResizableLayout({ pathname, temporaryUntil: "desktop" });
```

### Temporary Until Custom Media
...
```diff
 export function ExpandableLayoutExample(
   ...
   const {
     ...
     expandableNavProps,
-  } = useExpandableLayout({ pathname });
+  } = useExpandableLayout({ pathname, temporaryUntil: "screen and (min-width: 1200px)" });
```
...
```diff
 export function ResizableLayoutExample(
   ...
   const {
     ...
     windowSplitterProps,
-  } = useResizableLayout({ pathname });
+  } = useResizableLayout({ pathname, temporaryUntil: "screen and (min-width: 1200px)" });
```
