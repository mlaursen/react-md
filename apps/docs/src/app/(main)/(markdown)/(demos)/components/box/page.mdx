...
> `$box-gap` and `$box-padding` Sass variables.
...
> `$box-item-min-size` Sass variable.
...
configured by setting the `$box-grids` variable when `@forward`-ing or
...
`box-set-var` mixin or a custom class name on the `Box` component.
