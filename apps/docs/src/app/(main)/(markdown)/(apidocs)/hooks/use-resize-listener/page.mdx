...
The `useResizeListener` hook can be used to update React component functionality based
on window resize events with a few performance optimizations.

> !Info! The [useWindowSize](./use-window-size) hook can be used for most cases
> unless additional behavior is required.
